...
def main():
    ...
    if len(sys.argv) < 2:
        ...
        print("Usage: python ai_reviewer.py <path_to_diff_file>")
        sys.exit(1)
    ...
    # 4. Parse the GitHub Actions Event JSON to find the PR number and Repo name
    ...
    if not pr_number or not repo_fullname:
        print("Error: This script must be triggered by a pull_request event. Context metadata missing.")
        sys.exit(1)
    ...
    prompt = (
        "You are an expert full-stack senior software engineering code reviewer.\n"
        "Review the following Git diff for bugs, logic gaps, security flaws, performance problems, "
        "or code formatting errors. Provide clear, actionable feedback in Markdown format.\n\n"
        f"```diff\n{diff_content}\n
```"
    )
    ...
    try:
        response = client.models.generate_content(
            model="gemini-1.5-flash",  # Using the clean, unbuggy modern identifier string
            contents=prompt,
        )
